...
use VGMdb\Component\Form\Event\PostBindRequestEvent;
use VGMdb\Component\Form\Event\PostBindSavedDataEvent;
use VGMdb\Component\Form\Event\PostValidateEvent;
use VGMdb\Component\Form\Event\PreBindEvent;
use VGMdb\Component\Form\Storage\StorageInterface;
...
/**
 * Builds and handles multi-step forms.
 *
...
class FormFlow
{
    ...
    public function getStepDescriptions()
    {
        ...
        if ($stepDescriptionsCount > 0 && $stepDescriptionsCount !== $this->maxSteps) {
            throw new \RuntimeException(sprintf(
                'The number of steps (%u) doesn\'t match the number of step descriptions (%u). Either update the descriptions or remove them.',
                $this->maxSteps,
                $stepDescriptionsCount
            ));
        }
    ...
    protected function createFormForStep($formData, $step, array $options = array())
    {
        if (!$this->formType instanceof FormTypeInterface) {
            throw new \RuntimeException(sprintf(
                'The form type has to be an instance of type "%s", but "%s" given.',
